...
def load_data():
	...
	with open("rating.csv", "rb") as f:
		...
		for i, row in enumerate(reader):
			...
			elif counter < 1050000:
				row = map(int, row)
				if testing.has_key(row[0]):
					bisect.insort_left(testing[row[0]],row[1])
				else:
					testing[row[0]] = [row[1]]
			counter += 1
				

	return {'training': user_dict, 'testing': testing}
...
def testing(train, test):
	...
	for key, value in test.iteritems():
		if len(value) < 8:
			...
			user_test = value
			for i in range(0, 7):
				random_choice = random.randrange(len(user_test))
				temp = user_test[random_choice]
				guess.append(temp)
				del user_test[random_choice]
			best_fit = recommendation_list(train, user_test)
			best = selections(best_fit, user_test)
			user_recommendation = non_overlapping(best, user_test, train)
			result = shorter_dict(user_recommendation)
			for i in range(0, 7):
				temp = guess[i]
				if temp in result:
					correct += 1
...
dict_total = load_data()

training_dict = train_dict(dict_total)
testing_dict = test_dict(dict_total)
...
testing(training_dict, testing_dict)
